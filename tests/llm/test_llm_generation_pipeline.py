...
def create_test_document() -> Document:
    return Document(
        content=(
            ...
            "similarity search. "
            f"LLM generation pipeline test {uuid4()}."
        ),
        metadata=DocumentMetadata(
            filename="llm-generation-pipeline-test.pdf",
            title="LLM Generation Pipeline Test",
            author="Test Author",
            language="en",
            page_count=1,
        ),
...
"""
Given: knowledge stored with real local embeddings
When: a real query passes through retrieval, prompt augmentation,
and local LLM generation
Then: a non-empty grounded answer is generated
"""
def test_complete_real_rag_generation_pipeline():
